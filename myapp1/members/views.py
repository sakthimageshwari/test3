...
def science(request):
    ...
    if "submit2" in request.GET:
        ...
        template=loader.get_template('science.html')
        a=0
        if "id_text" in request.GET:
            a=int(request.GET.get("id_text",1))       
        if "op" in request.GET:
            check=request.GET["op"]
            correct=Q_S.objects.filter(id=a).values_list('answer',flat=True).first()
            if check==correct:
                score+=1
        a+=1   
        elements = Q_S.objects.filter(id=a).values()
